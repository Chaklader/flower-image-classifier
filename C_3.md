...
$h'(a_i^{(L)}) = \frac{d}{da_i^{(L)}}(a_i^{(L)} - y_i)$ $= \frac{d}{da_i^{(L)}}(a_i^{(L)}) - \frac{d}{da_i^{(L)}}(y_i)$
...
This derivative $h'(a_i^{(L)}) = 1$ tells us that a small change in $a_i^{(L)}$ causes an equal-sized change in the value of $h(a_i^{(L)})$.
