...
1. **Forward Pass**: Compute activations layer by layer from input to output. In the forward pass, we compute activations through each layer of the network:
   ...
   Here, $g^{(1)}$ is the activation function for the first layer and $\mathbf{a}^{(1)}$ is the output of the first later. Activation functions introduce non-linearity and allow the network to learn complex patterns. Common choices include:
...
2. **Calculate Output Error**: Determine how much each output neuron contributed to the total error.
...
Backpropagation is the algorithm that efficiently calculates gradients in neural networks by working backward from the
output.    
...
This simplification occurs because the derivatives of the softmax and cross-entropy cancel out certain terms, leading to
this elegant form.
...
###### Step 1: Find the Gradient of Loss with Respect to Output

To calculate how the loss changes when the network output changes, we need to find $\nabla_{\mathbf{a}^{(L)}}J$, which
is the vector of partial derivatives of the loss with respect to each output neuron.
...
###### Step 1.1: Isolate the terms that involve $a_i^{(L)}$

The loss function sums over all output neurons, but when we differentiate with respect to a specific output neuron $a_i^{(L)}$, only the term involving that neuron will contribute to the derivative. All other terms (where $j \neq i$) will have a derivative of zero.
...
###### Step 1.2: Apply the chain rule to the remaining term
...
- $f(u) = \frac{1}{2}u^2$ with $f'(u) = u$
- $g(a_i^{(L)}) = a_i^{(L)} - y_i$ with $g'(a_i^{(L)}) = 1$

Let me explain how we determine that $g'(a_i^{(L)}) = 1$ in this context.

In the derivation, I'm using the chain rule to find the derivative of a composite function. Here's what's happening:

We have the expression $\frac{1}{2}(a_i^{(L)} - y_i)^2$, and we're trying to differentiate it with respect to $a_i^{(L)}$.

To make this easier, I introduced two functions:

- $f(u) = \frac{1}{2}u^2$
- $g(a_i^{(L)}) = a_i^{(L)} - y_i$

So our original expression becomes $f(g(a_i^{(L)}))$, which we can differentiate using the chain rule.

Now, to find $g'(a_i^{(L)})$, we need to take the derivative of $g(a_i^{(L)}) = a_i^{(L)} - y_i$ with respect to $a_i^{(L)}$.
...
$g'(a_i^{(L)}) = \frac{d}{da_i^{(L)}}(a_i^{(L)} - y_i)$ $= \frac{d}{da_i^{(L)}}(a_i^{(L)}) - \frac{d}{da_i^{(L)}}(y_i)$

The derivative of $a_i^{(L)}$ with respect to itself is 1. This is the basic derivative rule that $\frac{d}{dx}(x) = 1$.

The derivative of $y_i$ with respect to $a_i^{(L)}$ is 0, because $y_i$ is a constant (target value) that doesn't depend on $a_i^{(L)}$. This follows the rule that the derivative of any constant is zero: $\frac{d}{dx}(c) = 0$.

Therefore: $g'(a_i^{(L)}) = 1 - 0 = 1$

This is why we state that $g'(a_i^{(L)}) = 1$ in our derivation. This derivative tells us that a small change in $a_i^{(L)}$ causes an equal-sized change in the value of $g(a_i^{(L)})$.
...
To see why the $\frac{1}{2}$ and $2$ cancel out, let's expand the derivative explicitly:

$$\frac{\partial}{\partial a_i^{(L)}} \left( \frac{1}{2}(a_i^{(L)} - y_i)^2 \right) = \frac{1}{2} \cdot \frac{\partial}{\partial a_i^{(L)}}(a_i^{(L)} - y_i)^2$$

Using the power rule from calculus: $\frac{d}{dx}(x^n) = nx^{n-1}$:

$$\frac{1}{2} \cdot \frac{\partial}{\partial a_i^{(L)}}(a_i^{(L)} - y_i)^2 = \frac{1}{2} \cdot 2(a_i^{(L)} - y_i) \cdot \frac{\partial}{\partial a_i^{(L)}}(a_i^{(L)} - y_i)$$

Since $\frac{\partial}{\partial a_i^{(L)}}(a_i^{(L)} - y_i) = 1$:

$$\frac{1}{2} \cdot 2(a_i^{(L)} - y_i) \cdot 1 = (a_i^{(L)} - y_i)$$
...
Which represents the difference between our predicted outputs and the target values. This vector indicates how much each output neuron needs to change to reduce the error, and it becomes the starting point for the backward propagation of error through the network.

This is a vector where each element is the difference between the predicted and actual value for that output neuron.

###### Step 2: Calculate Error Term Using the Chain Rule
...
derivatives of the softmax and cross-entropy combine in a way that many terms cancel out, giving us the elegant result:
...
without needing to explicitly calculate the derivative of the softmax function. This simplification is one reason why
softmax with cross-entropy is such a popular choice for classification problems - it leads to cleaner and more
computationally efficient backpropagation.

Understanding this error calculation is crucial because it's the starting point for the entire backpropagation process.
All the errors in earlier layers are derived from this initial output error through successive application of the chain
rule as we propagate backward through the network.
