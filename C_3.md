...
1. **Forward Pass**: Compute activations layer by layer from input to output.
   ...
   $\mathbf{a}^{(2)} = g^{(2)}(\mathbf{z}^{(2)})$
    ...
    And so on until the output layer $L$: $\mathbf{z}^{(L)} = \mathbf{W}^{(L)}\mathbf{a}^{(L-1)} + \mathbf{b}^{(L)}$
...
Here, $g^{(1)}$ is the activation function for the first layer. Activation functions introduce non-linearity and allow
the network to learn complex patterns. Common choices include:
