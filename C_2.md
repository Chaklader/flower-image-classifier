...
&= \frac{e^{z_i} / \sum e^{z_j}}{1 - e^{z_i} / \sum e^{z_j}} \\
&= S_i \cdot (1 - S_i)
\end{align}
$$
...
We get the same result, but without risking numerical overflow. The softmax function is typically used in the output
layer of neural networks for multi-class classification. It transforms the network's raw outputs into interpretable
probabilities that guide decision-making and provide insight into the model's confidence levels across different
classes.
