...
Let's derive this step by step starting with the definition:
...
This elegant relationship makes computing the derivative very efficient during neural network training—we can calculate
...
properties connect throughout neural network theory. However, this same relationship reveals the sigmoid's greatest
weakness: when the input has a large magnitude (either very positive or very negative), the derivative approaches zero.
For example:

- When $x = 6$, $\sigma(x) \approx 0.998$, so $\sigma'(x) \approx 0.002$
- When $x = -6$, $\sigma(x) \approx 0.002$, so $\sigma'(x) \approx 0.002$
...
essentially reducing the gradient to zero before it reaches the early layers. Despite this limitation, the sigmoid
function still finds use in specific contexts:
...
Several approaches can address this issue, for example, Leaky ReLU allows a small gradient for negative inputs:
...
5. **Gaussian Error Linear Unit (GELU)**: Approximates $$x \cdot \Phi(x)$$ where $$\Phi(x)$$ is the standard normal
   cumulative distribution function. GELU can be viewed as a smooth interpolation between ReLU and Leaky ReLU that has
   shown strong performance in modern transformer architectures.

To visualize how these functions differ, imagine four inputs: $-2, -0.5, 0, \ \text{and } \ 2$:
...
Visually, $\tanh$ creates an S-shaped curve similar to the sigmoid, but stretched vertically to range from $-1$ to $1$
instead of $0$ to $1$. In fact, there's a direct relationship between sigmoid and $\tanh$:

$$
\tanh(x) = 2\sigma(2x) - 1
$$

This shows that $\tanh$ is essentially a rescaled and shifted version of the sigmoid function. Let's examine the key
properties of $\tanh$:

1. **Bounded output (-1 to 1)**: Like sigmoid, $\tanh$ squashes inputs to a finite range, but centered at zero:

    - When $ x = 0, \tanh(x) = 0$
    - As $ x $ approaches $ +\infty $, $\tanh(x)$ approaches +1
    - As $ x $ approaches $ -\infty $, $\tanh(x)$ approaches -1
...
3. **Stronger gradients**: The derivative of $\tanh$ reaches a maximum value of $1$ (at $x = 0$), whereas the maximum
   derivative of sigmoid is only $0.25$. This generally allows for faster learning with $\tanh$.

The derivative of $\tanh$ can also be expressed in terms of the function itself:
...
Like the sigmoid, $\tanh$ also suffers from the vanishing gradient problem for inputs with large absolute values. When
$x$ has a large magnitude, $\tanh(x)$ approaches either 1 or -1, causing the derivative to approach zero. This can slow
down learning in deep networks.

Historically, $\tanh$ was often preferred over sigmoid for hidden layers due to its zero-centered nature and stronger
gradients. In modern deep learning, however, ReLU and its variants have largely replaced $\tanh$, except in specific
architectures:
...
The choice between sigmoid, $\tanh$, and ReLU often depends on the specific requirements of the neural network
architecture and the nature of the problem being solved.

##### Softmax for Multi-Class Problems

While sigmoid and $\tanh$ work well for binary problems or activating individual neurons, multi-class classification
...
assigned to the class with the highest initial score. The softmax function has several important properties that make it
...
because changing one input affects all outputs (due to the normalization). The derivative whch is the Gradient of
Softmax takes the form (Jacobian Matrix):

$$
\frac{\partial \sigma(z_i)}{\partial z_j} = \begin{cases} \sigma(z_i)(1 - \sigma(z_i)) & \text{if } i = j \\ -\sigma(z_i)\sigma(z_j) & \text{if } i \neq j \end{cases}
$$

Using the Kronecker delta $\delta_{ij}$ (which equals 1 if $i = j$ and 0 otherwise):
...
Cross-entropy is a mathematical concept that measures the difference between two probability distributions. In
information theory, it quantifies the average number of bits needed to encode data from one distribution using a coding
scheme optimized for another distribution.

Mathematically, for discrete probability distributions $P$ (true) and $Q$ (predicted), cross-entropy is defined as:
...
###### Why Cross-Entropy is Fundamental in Machine Learning

**1. Natural Loss Function for Probability Outputs**

Cross-entropy is the natural choice when your model outputs probabilities because:

- It directly measures how "surprised" we are by the model's predictions
- Lower cross-entropy means the model's predicted probabilities are closer to the true distribution
- It's undefined when the model predicts 0 probability for the correct class, forcing the model to never be completely
  confident in wrong predictions

**2. Mathematical Properties that Aid Optimization**

Cross-entropy has several properties that make it ideal for gradient descent:

- **Convex for linear models**: Guarantees a global minimum
- **Smooth gradient**: Provides stable gradient descent
- **Large gradients for wrong predictions**: When the model is very wrong, gradients are large, leading to faster
  learning
- **Small gradients near correct predictions**: Prevents overshooting when the model is already performing well

**3. Connection to Maximum Likelihood Estimation**

Minimizing cross-entropy is equivalent to maximizing the likelihood of the training data under the model. This provides
a principled statistical foundation for the learning process.

**4. Information-Theoretic Interpretation**

Cross-entropy measures the inefficiency of using the model's predicted distribution to encode the true distribution.
Minimizing this inefficiency leads to better predictions.

###### Practical Example

Consider a binary classifier predicting whether an email is spam:

- **True label**: $y = 1$ (spam)
- **Model prediction**: $\hat{y} = 0.9$ (90% confident it's spam)

Cross-entropy loss: $-[1 \cdot \log(0.9) + 0 \cdot \log(0.1)] = -\log(0.9) \approx 0.105$

If the model had predicted $\hat{y} = 0.1$ (10% confident): Cross-entropy loss: $-\log(0.1) \approx 2.303$

The loss is much higher when the model is confident but wrong, which drives the learning process effectively.

###### When to Use Cross-Entropy

Cross-entropy is the standard choice for:

- **Classification problems** (binary and multi-class)
- **Any scenario where outputs represent probabilities**
- **When you want probabilistic interpretations** of model confidence
- **Logistic regression and neural networks** with softmax outputs

Cross-entropy's combination of mathematical elegance, optimization properties, and intuitive interpretation makes it the
gold standard for classification tasks in machine learning.
...
function is commonly denoted with $\sigma$ (sigma), especially when dealing with multi-class classification.

**Why This Notation is Better:**

1. **Standard convention**: $\sigma$ is widely recognized for softmax in machine learning literature
2. **Consistency**: Matches the notation used for sigmoid function $\sigma(x) = \large \frac{1}{1+e^{-x}}$
...
Now let's derive why the gradient of cross-entropy loss with respect to softmax inputs simplifies so elegantly. The
...
This comes from differentiating the **cross-entropy loss function** with respect to the softmax outputs. The multi-class
...
All other terms $-y_j \log(S_j)$ where $j \neq i$ have derivative zero with respect to $S_i$. Using the basic derivative
rule $\frac{d}{dx}(\log(x)) = \frac{1}{x}$:
...
3. **Preserves ranking**: The class with the highest raw score will also have the highest probability after softmax. If
   $z_2 > z_8$ in the raw scores, then $\sigma(z_2) > \sigma(z_8)$ in the probabilities.
...
| Scenario | Model Prediction    | True Label        | Predicted Probabilities | Cross-Entropy Calculation | Loss Value | Interpretation                            |
| -------- | ------------------- | ----------------- | ----------------------- | ------------------------- | ---------- | ----------------------------------------- |
| **1**    | Confident & Correct | Class 1 (index 0) | [0.9, 0.05, 0.05]       | $-\log(0.9)$              | **0.105**  | Low loss - model is confident and correct |
| **2**    | Uncertain           | Class 1 (index 0) | [0.4, 0.3, 0.3]         | $-\log(0.4)$              | **0.916**  | Medium loss - model lacks confidence      |
| **3**    | Confident & Wrong   | Class 1 (index 0) | [0.1, 0.8, 0.1]         | $-\log(0.1)$              | **2.303**  | High loss - model is confident but wrong  |

###### Key Insights from the Table

**Loss Ranking (Low to High):**

1. **Confident + Correct** (0.105) - Best scenario
2. **Uncertain** (0.916) - Suboptimal but not catastrophic
3. **Confident + Wrong** (2.303) - Worst scenario

**Why This Pattern Matters:**

- Cross-entropy **heavily penalizes confident wrong predictions** (Scenario 3)
- It **moderately penalizes uncertainty** even when correct (Scenario 2)
- It **minimally penalizes confident correct predictions** (Scenario 1)

This behavior encourages the model to:

- Be confident when it's right
- Be uncertain rather than confidently wrong
- Learn from mistakes through large gradient signals when predictions are both confident and incorrect

The dramatic difference between Scenario 1 (0.105) and Scenario 3 (2.303) demonstrates why cross-entropy is so effective
at driving learning - wrong confident predictions generate strong learning signals.
...
For a dataset of $N$ samples and $K$ number of classes, the loss is typically averaged:
...
The second equation comes from applying the **multivariate chain rule** because in softmax, changing one logit $z_i$
affects **all** probabilities, not just $p_i$.

###### Why We Need the Full Chain Rule

In softmax, each probability depends on **all** logits:

$$
p_k = \frac{e^{z_k}}{\sum_{j=1}^{K} e^{z_j}}
$$

When we change $z_i$, it affects:

1. **Direct effect**: Changes the numerator if $k = i$
2. **Indirect effect**: Changes the denominator for **all** probabilities

###### Step-by-Step Derivation

**Starting point**: We want $\frac{\partial L_{CE}}{\partial z_i}$
...
But $L$ depends on $z_i$ through **multiple paths**:

- $L$ depends on $p_1, p_2, ..., p_K$ (all probabilities)
- Each $p_k$ depends on $z_i$ (because of the shared denominator in softmax)
...
**Part 1: Direct effect on $p_i$**

$$
\frac{\partial L_{CE}}{\partial p_i} \cdot \frac{\partial p_i}{\partial z_i}
$$

**Part 2: Indirect effects on all other probabilities**
...
If we only considered the direct effect $\frac{\partial L_{CE}}{\partial p_i} \cdot \frac{\partial p_i}{\partial z_i}$,
we'd miss the fact that changing $z_i$ also affects $p_1, p_2, ..., p_{i-1}, p_{i+1}, ..., p_K$ through the
normalization constraint $\sum p_k = 1$.

The second equation captures this **complete dependency structure** where one input affects all outputs in the softmax
function.
...
parameter change on the loss function, which is essential for proper optimization during training. Let's drop the sample
index $j$ for clarity. Let's calculate each part:
...
Therefore, we have the finalized form:
...
###### How the Simple Gradient Makes Training Effective

The elegantly simple gradient $\frac{\partial L_{CE}}{\partial z_i} = p_i - y_i$ is incredibly useful because it
provides **direct, proportional error correction** that automatically guides the model toward better predictions.

###### The Error Correction Mechanism

**When prediction is too high** ($p_i > y_i$):

- **Gradient**: Positive value $(p_i - y_i > 0)$
- **Weight update**: $z_i \leftarrow z_i - \alpha(p_i - y_i)$ (decreases $z_i$)
- **Effect**: Lower logit → lower probability → moves prediction toward target

**When prediction is too low** ($p_i < y_i$):

- **Gradient**: Negative value $(p_i - y_i < 0)$
- **Weight update**: $z_i \leftarrow z_i - \alpha(p_i - y_i)$ (increases $z_i$)
- **Effect**: Higher logit → higher probability → moves prediction toward target

**When prediction is correct** ($p_i = y_i$):

- **Gradient**: Zero $(p_i - y_i = 0)$
- **Weight update**: No change
- **Effect**: Perfect prediction requires no adjustment

###### Proportional Correction Power

The gradient magnitude reflects the **severity of the error**:

**Large errors get large corrections:**

- If $p_i = 0.1$ but $y_i = 1$ (terrible prediction), gradient = $-0.9$ (strong correction)
- If $p_i = 0.9$ but $y_i = 0$ (overconfident wrong), gradient = $+0.9$ (strong correction)

**Small errors get small corrections:**

- If $p_i = 0.8$ but $y_i = 1$ (good but not perfect), gradient = $-0.2$ (gentle correction)
- If $p_i = 0.2$ but $y_i = 0$ (mostly correct), gradient = $+0.2$ (gentle correction)

###### Computational Efficiency Benefits

**1. No Complex Derivatives:**

- No need to compute complicated chain rule through softmax and cross-entropy separately
- Direct computation: just subtract predicted from actual
...
G = gradients, P = predictions, and T = targets

**3. Numerical Stability:**

- No divisions by small probabilities (which could cause numerical issues)
- No exponential computations needed for gradients
- Stable across all probability ranges

###### Intuitive Learning Behavior

The gradient provides **meaningful learning signals**:

**For correct class** ($y_i = 1$):

- If $p_i = 0.3$ → gradient = $-0.7$ → "Increase this class probability significantly"
- If $p_i = 0.9$ → gradient = $-0.1$ → "Increase this class probability slightly"

**For incorrect classes** ($y_i = 0$):

- If $p_i = 0.7$ → gradient = $+0.7$ → "Decrease this class probability significantly"
- If $p_i = 0.1$ → gradient = $+0.1$ → "Decrease this class probability slightly"

###### Automatic Balancing Across Classes

Because softmax probabilities sum to $1$, when the gradient pushes one class probability up, it automatically pushes
others down. The simple gradient form ensures this happens in exactly the right proportions without additional
computation.

###### Comparison with Alternatives

**If we used mean squared error** with softmax:

- Gradient would be: $(p_i - y_i) \times p_i \times (1 - p_i)$
- Problems:
    - More complex computation
    - Gradient vanishes when $p_i$ near $0$ or $1$
    - Less effective learning for confident wrong predictions

**The cross-entropy + softmax combination**:

- Maintains strong gradients even for confident predictions
- Provides linear error correction proportional to mistake magnitude
...
- Direct error signals mean fewer training iterations needed
- No wasted computation on complex gradient calculations

**Stable Training:**

- Gradients naturally bounded between $-1$ and $+1$
- No exploding gradient problems
- Consistent learning rate effectiveness

**Effective Error Correction:**

- Large mistakes get corrected quickly
- Small mistakes get fine-tuned precisely
- Perfect predictions stay unchanged

This simple gradient form is one of the key reasons why softmax + cross-entropy became the standard for multi-class
classification—it provides the most direct and effective learning signal possible, making both implementation and
training remarkably efficient.
...
behavior. In binary classification, the decision boundary is a single hypersurface, but multi-class classification
involves multiple boundaries that collectively partition the feature space into regions corresponding to different
classes.
...
<div align="center">
<img src="images/AI_Programming_with_Python_ND_P2_C_2_07.png" width="400" height="auto">
<p style="color: #555;">Figure:  Decision Boundaries in Higher Dimensions</p>
</div>
...
<div align="center">
<img src="images/AI_Programming_with_Python_ND_P2_C_2_08.png" width="400" height="auto">
<p style="color: #555;">Figure:  Decision Boundaries in Higher Dimensions</p>
</div>
...
much more informative. The logistic regression model has an interesting interpretation when we examine the log-odds (or
logit) of the positive class:
...
The equation shows how the **log-odds** (logit) in logistic regression equals the linear predictor. Here's the
step-by-step derivation:

###### Starting with Logistic Regression Definition
...
The **odds ratio** is a way to express how likely one outcome is compared to another outcome. It's different from just
probability.

###### What "Odds" Means

**Odds** compares the probability of something happening to the probability of it NOT happening:
...
- **Numerator**: Probability of passing the exam
- **Denominator**: Probability of failing the exam
- **Ratio**: How many times more likely to pass than fail
...
- $P(fail) = 1 - 0.8 = 0.2$
- $\text{Odds} = 0.8/0.2 = 4$
- **Interpretation**: "4 times more likely to pass than fail" or "4:1 odds in favor of passing"

**If $P(pass) = 0.6$:**

- $P(fail) = 1 - 0.6 = 0.4$
- $\text{Odds} = 0.6/0.4 = 1.5$
- **Interpretation**: "1.5 times more likely to pass than fail"

**If $P(pass) = 0.5$:**

- $P(fail) = 1 - 0.5 = 0.5$
- $\text{Odds} = 0.5/0.5 = 1$
- **Interpretation**: "Equal chance of passing or failing" (even odds)

**If $P(pass) = 0.3$:**

- $P(fail) = 1 - 0.3 = 0.7$
- $\text{Odds} = 0.3/0.7 ≈ 0.43$
- **Interpretation**: "Less likely to pass than fail"

###### Why Use Odds Instead of Probability?

1. **Range**: Odds range from 0 to ∞, while probabilities range from 0 to 1
2. **Symmetry**: Odds of 4 (4:1 favor) and 0.25 (1:4 against) are symmetric around 1
3. **Multiplication**: When you take the log, multiplication becomes addition (useful for modeling)

The odds ratio is the foundation for understanding why the **log-odds** (logit) becomes linear in logistic regression!

###### Step 2: Substitute the Logistic Function

$$
\frac{P(y = 1|\mathbf{x})}{1 - P(y = 1|\mathbf{x})} = \frac{\frac{1}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}}{1 - \frac{1}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}}
$$

###### Step 3: Simplify the Denominator

$$
1 - \frac{1}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}} = \frac{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}} - 1}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}} = \frac{{\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}
$$
...
$$
\begin{align}
\frac{P(y = 1|\mathbf{x})}{1 - P(y = 1|\mathbf{x})} &= \frac{\frac{1}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}}{\frac{{\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}} \\
&= \frac{1}{{\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}} = {\Large e^{\mathbf{w}^T\mathbf{x} + b}}
\end{align}
$$
...
$$
\begin{align}
\log\left(\frac{P(y = 1|\mathbf{x})}{P(y = 0|\mathbf{x})}\right) &= \log\left(e^{\mathbf{w}^T\mathbf{x} + b}\right) = \mathbf{w}^T\mathbf{x} + b
\end{align}
$$

###### The Key Insight

This shows that in logistic regression:

- The **log-odds** (logit) is **linear** in the features
- The **probabilities** are **non-linear** (S-shaped curve)
- The **linear predictor** $\mathbf{w}^T\mathbf{x} + b$ directly represents log-odds

This linear relationship in log-odds space is what makes logistic regression both interpretable and computationally
tractable!
...
$$
P(y=1|\mathbf{x}) = \sigma(\mathbf{w}^T\mathbf{x} + b) = \frac{1}{1 + {\Large e^{-(\mathbf{w}^T\mathbf{x} + b)}}}
$$
...
theoretical foundations and makes its predictions particularly useful for decision-making under uncertainty. The
logistic regression model directly outputs the probability that an instance belongs to the positive class:
...
###### Deriving the Compact Probability Expression

The last line is a **compact way to write both probabilities** (positive and negative class) in a single formula using
the actual class label $y$ as an exponent.

###### The Logic Behind the Compact Form

We have two separate probability expressions:

- When $y = 1$: $P(y = 1|\mathbf{x}; \mathbf{w}, b) = \sigma(\mathbf{w}^T\mathbf{x} + b)$
- When $y = 0$: $P(y = 0|\mathbf{x}; \mathbf{w}, b) = 1 - \sigma(\mathbf{w}^T\mathbf{x} + b)$

###### The Clever Trick

The compact form uses $y$ as an exponent to automatically select the correct probability:
...
- $y = 1$ and $(1-y) = 0$
- $P(y=1|\mathbf{x}) = \sigma(\mathbf{w}^T\mathbf{x} + b)^1 \cdot (1 - \sigma(\mathbf{w}^T\mathbf{x} + b))^0$
- $= \sigma(\mathbf{w}^T\mathbf{x} + b) \cdot 1$
- $= \sigma(\mathbf{w}^T\mathbf{x} + b)$ ✅

**Case 2: When $y = 0$ (negative class)**

- $y = 0$ and $(1-y) = 1$
- $P(y=0|\mathbf{x}) = \sigma(\mathbf{w}^T\mathbf{x} + b)^0 \cdot (1 - \sigma(\mathbf{w}^T\mathbf{x} + b))^1$
- $= 1 \cdot (1 - \sigma(\mathbf{w}^T\mathbf{x} + b))$
- $= 1 - \sigma(\mathbf{w}^T\mathbf{x} + b)$ ✅

###### Why This is Useful

**Advantages of the compact form:**

1. **Single equation** handles both classes
2. **Elegant for likelihood functions** - you can write the likelihood for all data points as a product
3. **Mathematically convenient** for taking derivatives
4. **Common in statistical literature** and machine learning textbooks

**Example usage in likelihood:**

$$
L(\mathbf{w}, b) = \prod_{i=1}^{n} P(y^{(i)}|\mathbf{x}^{(i)}; \mathbf{w}, b) = \prod_{i=1}^{n} \sigma(\mathbf{w}^T\mathbf{x}^{(i)} + b)^{y^{(i)}} \cdot (1 - \sigma(\mathbf{w}^T\mathbf{x}^{(i)} + b))^{(1-y^{(i)})}
$$

This compact notation elegantly captures the entire logistic regression probability model in one expression!
...
only have two cases ($y=0$ and $y=1$), we can create an expression that selects the right formula based on the value of
y.
...
The likelihood function is a fundamental concept in statistics that often causes confusion. Let's explain what it means
both mathematically and intuitively. When we say "the likelihood function represents the probability of observing our
entire dataset given specific model parameters," we're referring to a mathematical function that measures how well a
particular set of parameters explains our observed data.
...
&= \prod_{i=1}^{n} \sigma(\mathbf{w}^T\mathbf{x}^{(i)} + b)^{y^{(i)}} \cdot (1 - \sigma(\mathbf{w}^T\mathbf{x}^{(i)} + b))^{(1-y^{(i)})}
\end{align}
$$
...
###### Understanding the Log-Likelihood Transformation

This shows how we transform the **likelihood function** into the **log-likelihood function** using logarithm properties.

###### Starting Point: The Likelihood Function
...
- For each example $i$, we compute $P(y^{(i)}|\mathbf{x}^{(i)}; \mathbf{w}, b)$ (probability of the observed label)
...
**Logarithm of a product becomes sum of logarithms:** $$\log(a \times b \times c) = \log(a) + \log(b) + \log(c)$$

More generally: $$\log \prod_{i=1}^{m} a_i = \sum_{i=1}^{m} \log a_i$$

###### Applying This Property

$$
\log \prod_{i=1}^{m} P(y^{(i)}|\mathbf{x}^{(i)}; \mathbf{w}, b) = \sum_{i=1}^{m} \log P(y^{(i)}|\mathbf{x}^{(i)}; \mathbf{w}, b)
$$
...
1. **Numerical Stability**: Products of small probabilities can cause underflow; sums of logs are more stable
2. **Easier Optimization**: Addition is easier to differentiate than multiplication
3. **Convexity**: Log-likelihood is often concave, making optimization easier

**Mathematical Convenience:**

- **Monotonic**: $\log$ is monotonic, so maximizing $\ell$ is equivalent to maximizing $L$
- **Additive**: Each training example contributes independently to the total log-likelihood
- **Differentiable**: Easier to compute gradients for optimization

**Example:** Instead of: $L = 0.8 \times 0.6 \times 0.9 = 0.432$ We get:
$\ell = \log(0.8) + \log(0.6) + \log(0.9) = -0.223 - 0.511 - 0.105 = -0.839$

The transformation from product to sum is the fundamental step that makes maximum likelihood estimation computationally
tractable!
...
Implementing logistic regression for binary classification involves several key steps, from preparing the data to making
predictions with the trained model. Let's walk through a complete implementation, highlighting practical considerations
at each stage. Before training a logistic regression model, we need to ensure our data is properly prepared:
...
These gradients come from taking the partial derivatives of the cross-entropy loss function. Let me show the
step-by-step derivation.
...
###### Key Derivatives We Need

First, let's establish the derivative of the sigmoid function:

$$
\frac{d\sigma}{dz} = \sigma(z)(1 - \sigma(z))
$$

###### Gradient with Respect to Weights ($\frac{\partial J}{\partial w_j}$)

**Step 1: Apply chain rule**

$$
\frac{\partial J}{\partial w_j} = -\frac{1}{m} \sum_{i=1}^{m} \frac{\partial}{\partial w_j}\left[y^{(i)} \log \sigma(z^{(i)}) + (1 - y^{(i)}) \log(1 - \sigma(z^{(i)}))\right]
$$

**Step 2: Chain rule for each term** For the first term:

$$
\frac{\partial}{\partial w_j}[y^{(i)} \log \sigma(z^{(i)})] = y^{(i)} \cdot \frac{1}{\sigma(z^{(i)})} \cdot \frac{\partial \sigma}{\partial z^{(i)}} \cdot \frac{\partial z^{(i)}}{\partial w_j}
$$

For the second term:

$$
\frac{\partial}{\partial w_j}[(1-y^{(i)}) \log(1 - \sigma(z^{(i)}))] = (1-y^{(i)}) \cdot \frac{1}{1-\sigma(z^{(i)})} \cdot \frac{\partial (1-\sigma)}{\partial z^{(i)}} \cdot \frac{\partial z^{(i)}}{\partial w_j}
$$

This step shows how to apply the **chain rule** to differentiate the two terms in the cross-entropy loss function. Let
me break down each term.

###### First Term: $\frac{\partial}{\partial w_j}[y^{(i)} \log \sigma(z^{(i)})]$

**Chain rule breakdown:**

$$
\frac{\partial}{\partial w_j}[y^{(i)} \log \sigma(z^{(i)})] = y^{(i)} \cdot \frac{\partial}{\partial w_j}[\log \sigma(z^{(i)})]
$$

**Applying chain rule to $\log \sigma(z^{(i)})$:**

$$
\frac{\partial}{\partial w_j}[\log \sigma(z^{(i)})] = \frac{1}{\sigma(z^{(i)})} \cdot \frac{\partial \sigma}{\partial z^{(i)}} \cdot \frac{\partial z^{(i)}}{\partial w_j}
$$

**Breaking it down:**

- $\frac{\partial}{\partial \sigma}[\log \sigma] = \frac{1}{\sigma}$ (derivative of natural log)
- $\frac{\partial \sigma}{\partial z^{(i)}} = \frac{\partial \sigma}{\partial z^{(i)}}$ (derivative of sigmoid w.r.t.
  its input)
- $\frac{\partial z^{(i)}}{\partial w_j} = \frac{\partial z^{(i)}}{\partial w_j}$ (derivative of linear function w.r.t.
  weight)

**Complete first term:**

$$
y^{(i)} \cdot \frac{1}{\sigma(z^{(i)})} \cdot \frac{\partial \sigma}{\partial z^{(i)}} \cdot \frac{\partial z^{(i)}}{\partial w_j}
$$

###### Second Term: $\frac{\partial}{\partial w_j}[(1 - y^{(i)}) \log(1 - \sigma(z^{(i)}))]$

**Chain rule breakdown:**

$$
\frac{\partial}{\partial w_j}[(1 - y^{(i)}) \log(1 - \sigma(z^{(i)}))] = (1 - y^{(i)}) \cdot \frac{\partial}{\partial w_j}[\log(1 - \sigma(z^{(i)}))]
$$

**Applying chain rule to $\log(1 - \sigma(z^{(i)}))$:**

$$
\frac{\partial}{\partial w_j}[\log(1 - \sigma(z^{(i)}))] = \frac{1}{1 - \sigma(z^{(i)})} \cdot \frac{\partial(1-\sigma)}{\partial z^{(i)}} \cdot \frac{\partial z^{(i)}}{\partial w_j}
$$

**Breaking it down:**

- $\frac{\partial}{\partial u}[\log u] = \frac{1}{u}$ where $u = 1 - \sigma(z^{(i)})$
- $\frac{\partial(1-\sigma)}{\partial z^{(i)}} = -\frac{\partial \sigma}{\partial z^{(i)}}$ (derivative of $(1-\sigma)$)
- $\frac{\partial z^{(i)}}{\partial w_j} = \frac{\partial z^{(i)}}{\partial w_j}$ (same as before)

**Complete second term:**

$$
(1 - y^{(i)}) \cdot \frac{1}{1-\sigma(z^{(i)})} \cdot \frac{\partial(1-\sigma)}{\partial z^{(i)}} \cdot \frac{\partial z^{(i)}}{\partial w_j}
$$
...
$$
\frac{\partial}{\partial w_j}[f(g(h(w_j)))] = f'(g(h)) \cdot g'(h) \cdot h'(w_j)
$$

**For our case:**

- **Outer function**: $\log(\cdot)$ → derivative is $\frac{1}{\text{input}}$
- **Middle function**: $\sigma(\cdot)$ or $(1-\sigma(\cdot))$ → derivatives involve sigmoid derivatives
- **Inner function**: $z^{(i)} = \mathbf{w}^T\mathbf{x}^{(i)} + b$ → derivative w.r.t. $w_j$ gives $x_j^{(i)}$

###### Why This Step is Important

This careful application of the chain rule ensures we correctly account for:

1. **The logarithmic transformation** (outer layer)
2. **The sigmoid activation** (middle layer)
3. **The linear combination** (inner layer)

Each layer contributes its own derivative, and the chain rule multiplies them together to get the complete gradient.
This systematic approach is essential for correctly computing gradients in more complex neural networks.

**Step 3: Compute individual derivatives**

- $\frac{\partial z^{(i)}}{\partial w_j} = x_j^{(i)}$ (since $z^{(i)} = \mathbf{w}^T\mathbf{x}^{(i)} + b$)
- $\frac{\partial \sigma}{\partial z^{(i)}} = \sigma(z^{(i)})(1 - \sigma(z^{(i)}))$
- $\frac{\partial (1-\sigma)}{\partial z^{(i)}} = -\sigma(z^{(i)})(1 - \sigma(z^{(i)}))$

###### Computing the Individual Derivatives

This step calculates the specific derivatives needed for the chain rule. Let me explain each one.

###### 1. Derivative of the Linear Function: $\frac{\partial z^{(i)}}{\partial w_j} = x_j^{(i)}$

**Starting with the linear function:**

$$
z^{(i)} = \mathbf{w}^T\mathbf{x}^{(i)} + b = w_1 x_1^{(i)} + w_2 x_2^{(i)} + \cdots + w_j x_j^{(i)} + \cdots + w_n x_n^{(i)} + b
$$

**Taking the partial derivative with respect to $w_j$:**

- All terms without $w_j$ become zero
- The term $w_j x_j^{(i)}$ gives us $x_j^{(i)}$
- The bias term $b$ becomes zero

**Result:** $\frac{\partial z^{(i)}}{\partial w_j} = x_j^{(i)}$

This makes intuitive sense: changing weight $w_j$ affects the output $z^{(i)}$ proportionally to the corresponding
feature value $x_j^{(i)}$.

###### 2. Derivative of the Sigmoid Function: $\frac{\partial \sigma}{\partial z^{(i)}} = \sigma(z^{(i)})(1 - \sigma(z^{(i)}))$
...
**Using the quotient rule:** Let $u = 1$ and $v = 1 + e^{-z}$, so $\sigma = \frac{u}{v}$

$$
\frac{d\sigma}{dz} = \frac{u'v - uv'}{v^2} = \frac{0 \cdot (1 + e^{-z}) - 1 \cdot (-e^{-z})}{(1 + e^{-z})^2} = \frac{e^{-z}}{(1 + e^{-z})^2}
$$

**Converting to the elegant form:**
...
###### 3. Derivative of $(1 - \sigma)$: $\frac{\partial(1-\sigma)}{\partial z^{(i)}} = -\sigma(z^{(i)})(1 - \sigma(z^{(i)}))$

**This is straightforward:**

$$
\frac{\partial}{\partial z^{(i)}}[1 - \sigma(z^{(i)})] = 0 - \frac{\partial \sigma}{\partial z^{(i)}} = -\sigma(z^{(i)})(1 - \sigma(z^{(i)}))
$$

The negative sign comes from differentiating $(1 - \sigma)$ where the derivative of the constant $1$ is zero and the
derivative of $-\sigma$ gives us $-\frac{\partial \sigma}{\partial z}$.

###### Key Insights

**Pattern Recognition:**

- **Linear derivatives**: Give us the input features
- **Sigmoid derivatives**: Have the characteristic $\sigma(1-\sigma)$ form
- **Negative sigmoid derivatives**: Just add a negative sign

**Mathematical Beauty:** The sigmoid derivative $\sigma(z)(1-\sigma(z))$ has a beautiful property:

- It's maximum when $\sigma(z) = 0.5$ (at $z = 0$), giving $0.5 \times 0.5 = 0.25$
- It approaches zero as $\sigma(z)$ approaches $0$ or $1$ (saturation regions)

This explains why sigmoid can suffer from vanishing gradients in deep networks when neurons saturate!

**Step 4: Substitute and simplify**

$$
\begin{align}
\frac{\partial J}{\partial w_j} &= -\frac{1}{m} \sum_{i=1}^{m} \left[y^{(i)} \cdot \frac{\sigma(z^{(i)})(1 - \sigma(z^{(i)}))}{\sigma(z^{(i)})} - (1-y^{(i)}) \cdot \frac{\sigma(z^{(i)})(1 - \sigma(z^{(i)}))}{1-\sigma(z^{(i)})}\right] x_j^{(i)} \\
&= -\frac{1}{m} \sum_{i=1}^{m} \left[y^{(i)}(1 - \sigma(z^{(i)})) - (1-y^{(i)})\sigma(z^{(i)})\right] x_j^{(i)} \\
&= -\frac{1}{m} \sum_{i=1}^{m} \left[y^{(i)} - y^{(i)}\sigma(z^{(i)}) - \sigma(z^{(i)}) + y^{(i)}\sigma(z^{(i)})\right] x_j^{(i)} \\
&= -\frac{1}{m} \sum_{i=1}^{m} \left[y^{(i)} - \sigma(z^{(i)})\right] x_j^{(i)} \\
&= \frac{1}{m} \sum_{i=1}^{m} (\sigma(z^{(i)}) - y^{(i)}) x_j^{(i)}
\end{align}
$$

###### Gradient with Respect to Bias ($\frac{\partial J}{\partial b}$)

**Similar process, but simpler:** The only difference is that $\frac{\partial z^{(i)}}{\partial b} = 1$ instead of
$x_j^{(i)}$.
...
Both gradients have the form:

- **Error term**: $(\sigma(z^{(i)}) - y^{(i)})$ (prediction minus actual)
- **Input scaling**: Multiplied by the input feature (for weights) or $1$ (for bias)

This elegant result shows that the gradient is simply the **prediction error** weighted by the input features, making
logistic regression both interpretable and computationally efficient!
...
3. Batch Processing: For each mini-batch:
