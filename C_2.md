...
Or in Leibniz notation: $$\large \frac{dy}{dx} = \frac{dy}{du} \cdot \frac{du}{dx}$$ where $u = g(x)$ is the inner
function.
...
2. Find $f'(x)$ and $g'(x)$
3. Apply the formula: $\frac{f'(x)g(x) - f(x)g'(x)}{[g(x)]^2}$
...
rule: $$\frac{f(x)}{g(x)} = f(x) \cdot [g(x)]^{-1}$$

Then use the product rule: $(uv)' = u'v + uv'$
...
<div align="center">
<img src="images/AI_Programming_with_Python_ND_P2_C_2_02.png" width="600" height="auto">
<p style="color: #555;">Figure:   Fine-tuning Pre-trained Models</p>
</div>

##### Deriving the Sigmoid Function Derivative Step by Step

Let me walk through a more detailed derivation of why the sigmoid function's derivative equals the product of the
function and one minus the function: $σ'(x) = σ(x)(1-σ(x))$.

###### Starting Point: The Sigmoid Definition

We begin with the definition of the sigmoid function: $$σ(x) = \frac{1}{1+e^{-x}}$$

This is our starting point - a function that maps any real number to a value between 0 and 1.

###### Step 1: Apply the Chain Rule

To find the derivative, we apply the chain rule from calculus. The sigmoid is a composite function (a fraction), so
we'll need to use the quotient rule, which is a special case of the chain rule.

###### Step 2: Using the Quotient Rule

The quotient rule states that for a function $f(x) = \frac{g(x)}{h(x)}$, the derivative is:
$$f'(x) = \frac{g'(x)h(x) - g(x)h'(x)}{[h(x)]^2}$$

For the sigmoid:

- $g(x) = 1$ (the numerator is a constant)
- $h(x) = 1+e^{-x}$ (the denominator)

So:

- $g'(x) = 0$ (derivative of a constant is zero)
- $h'(x) = \frac{d}{dx}(1+e^{-x}) = \frac{d}{dx}(e^{-x}) = -e^{-x}$ (using chain rule on $e^{-x}$)

###### Step 3: Substituting into the Quotient Rule

$$σ'(x) = \frac{0 \cdot (1+e^{-x}) - 1 \cdot (-e^{-x})}{(1+e^{-x})^2}$$

The first term becomes zero since $g'(x) = 0$:
$$σ'(x) = \frac{0 - (-e^{-x})}{(1+e^{-x})^2} = \frac{e^{-x}}{(1+e^{-x})^2}$$

###### Step 4: Recognizing the Connection to $σ(x)$

Now we need to show that this equals $σ(x)(1-σ(x))$. Let's work with the original sigmoid: $$σ(x) = \frac{1}{1+e^{-x}}$$

And calculate $1-σ(x)$:
$$1-σ(x) = 1-\frac{1}{1+e^{-x}} = \frac{1+e^{-x}}{1+e^{-x}} - \frac{1}{1+e^{-x}} = \frac{1+e^{-x}-1}{1+e^{-x}} = \frac{e^{-x}}{1+e^{-x}}$$

###### Step 5: Multiplying $σ(x)$ and $(1-σ(x))$

$$σ(x)(1-σ(x)) = \frac{1}{1+e^{-x}} \cdot \frac{e^{-x}}{1+e^{-x}} = \frac{e^{-x}}{(1+e^{-x})^2}$$

###### Step 6: Confirming the Result

Looking at our derivative from Step 3: $$σ'(x) = \frac{e^{-x}}{(1+e^{-x})^2}$$

And our product from Step 5: $$σ(x)(1-σ(x)) = \frac{e^{-x}}{(1+e^{-x})^2}$$

We can see they're identical! Therefore: $$σ'(x) = σ(x)(1-σ(x))$$

This elegant relationship makes the sigmoid function particularly useful in neural networks because:
...
The fact that the exponential function e^u is its own derivative is one of the most elegant and important properties in
calculus. Let me explain why this is true through several different approaches.
...
Therefore: $$\frac{d}{du}(e^u) = e^u \cdot 1 = e^u$$

###### Understanding the Fundamental Limit That Defines Euler's Number
...
using the limit definition: $$\large \frac{d}{dx}(a^x) = \lim_{h \to 0} \frac{a^{x+h} - a^x}{h}$$

Using the property $a^{x+h} = a^x \cdot a^h$:
$$\large \frac{d}{dx}(a^x) = \lim_{h \to 0} \frac{a^x \cdot a^h - a^x}{h} = a^x \cdot \lim_{h \to 0} \frac{a^h - 1}{h}$$
...
**For $a = 2$:** $$\lim_{h \to 0} \frac{2^h - 1}{h} \approx 0.693$$

This means $\frac{d}{dx}(2^x) = 2^x \cdot 0.693$. The function $2^x$ grows, but not as fast as itself.

**For $a = 3$:** $$\lim_{h \to 0} \frac{3^h - 1}{h} \approx 1.099$$
...
$$\frac{d}{du}(e^u) = 0 + 1 + \frac{2u}{2!} + \frac{3u^2}{3!} + \frac{4u^3}{4!} + ...$$

$$\frac{d}{du}(e^u) = 1 + u + \frac{u^2}{2!} + \frac{u^3}{3!} + ...$$

This is exactly the Taylor series for e^u, confirming that: $$\frac{d}{du}(e^u) = e^u$$
...
3. It connects to the sigmoid function's derivative: σ'(x) = σ(x)(1-σ(x))
4. It means that we can easily find the derivative of e^x for any complex function x without memorizing additional
   formulas

This self-derivative property makes e^x the most natural base for exponential functions in calculus, just as base 10 is
natural for human counting and base 2 is natural for computing.

##### Understanding the Chain Rule for Differentiating $e^{-x}$

The chain rule is essential for finding the derivative of composite functions like $e^{-x}$. Let me walk through exactly
how it works in this case.

###### What is e^(-x)?

First, let's recognize that e^(-x) is a composite function. We can think of it as:

- The outer function: f(u) = e^u (the exponential function)
- The inner function: u(x) = -x (a linear function)

So e^(-x) = f(u(x)) = e^(u(x)) = e^(-x)
...
- dy/du = d(e^u)/du = e^u (the derivative of e^u is e^u)
- du/dx = d(-x)/dx = -1 (the derivative of -x is -1)

Therefore: dy/dx = e^u × (-1) = -e^u = -e^(-x)

###### Why This Makes Sense

The exponential function e^x always has a positive slope that increases with x. When we negate the input (e^(-x)), we're
essentially "flipping" the function horizontally, causing it to decrease as x increases. This is why the derivative
becomes negative.

The derivative tells us the rate of change at any point. Since e^(-x) decreases as x increases, its rate of change is
negative, explaining why we get -e^(-x).

###### Connecting to the Sigmoid Derivative

This result is crucial for deriving the sigmoid derivative. When we apply the quotient rule to σ(x) = 1/(1+e^(-x)), the
term -e^(-x) appears as the derivative of e^(-x), which then leads to the elegant form σ'(x) = σ(x)(1-σ(x)).
...
$$
f'(x) = \lim_{\Delta x \to 0} \frac{\frac{g(x + \Delta x)h(x) - g(x)h(x + \Delta x)}{h(x + \Delta x)h(x)}}{\Delta x}
$$
...
Differentiating both sides using the product rule: $f'(x) \cdot h(x) + f(x) \cdot h'(x) = g'(x)$

Solving for $f'(x)$: $f'(x) = \frac{g'(x) - f(x) \cdot h'(x)}{h(x)}$

Substituting $f(x) = \frac{g(x)}{h(x)}$:
$f'(x) = \frac{g'(x) - \frac{g(x)}{h(x)} \cdot h'(x)}{h(x)} = \frac{g'(x) - \frac{g(x)h'(x)}{h(x)}}{h(x)}$

Simplifying: $f'(x) = \frac{g'(x)h(x) - g(x)h'(x)}{[h(x)]^2}$
...
Imagine a rectangle with dimensions f(x) and g(x). The area of this rectangle is f(x) · g(x).
...
1. The change in width while keeping the height constant: f'(x) · g(x)
2. The change in height while keeping the width constant: f(x) · g'(x)

Adding these two contributions gives us the total rate of change: f'(x) · g(x) + f(x) · g'(x)
...
1. Hyperbolic sine: $$sinh(x) = (e^x - e^{-x})/2$$
2. Hyperbolic cosine: $$cosh(x) = (e^x + e^{-x})/2$$
3. Hyperbolic tangent: $$tanh(x) = (e^x - e^{-x})/(e^x + e^{-x})$$
4. Hyperbolic cosecant: $$csch(x) = 1/sinh(x)$$
5. Hyperbolic secant: $$sech(x) = 1/cosh(x)$$
6. Hyperbolic cotangent: $$coth(x) = 1/tanh(x)$$
...
- For points (x,y) on the unit circle, $$x = cos(θ)$$ and $$y = sin(θ)$$
- For points (x,y) on the unit hyperbola $$x² - y² = 1, x = cosh(t)$$ and $$y = sinh(t)$$
...
$$d/dx(sinh(x)) = cosh(x) d/dx(cosh(x)) = sinh(x) d/dx(tanh(x)) = 1 - tanh²(x)$$
...
**Derivative of Hyperbolic Sine:** $$\large \frac{d}{dx}(\sinh(x)) = \cosh(x)$$

**Proof:** Starting with the definition: $$\sinh(x) = \frac{e^x - e^{-x}}{2}$$

Differentiating term by term:
$$\frac{d}{dx}(\sinh(x)) = \frac{d}{dx}\left(\frac{e^x - e^{-x}}{2}\right) = \frac{1}{2}\left(\frac{d}{dx}(e^x) - \frac{d}{dx}(e^{-x})\right)$$

Using the chain rule for $e^{-x}$: $$= \frac{1}{2}(e^x - e^{-x} \cdot (-1)) = \frac{1}{2}(e^x + e^{-x}) = \cosh(x)$$

**Derivative of Hyperbolic Cosine:** $$\large \frac{d}{dx}(\cosh(x)) = \sinh(x)$$

**Proof:** Starting with the definition: $$\cosh(x) = \frac{e^x + e^{-x}}{2}$$

Differentiating: $$\frac{d}{dx}(\cosh(x)) = \frac{1}{2}\left(\frac{d}{dx}(e^x) + \frac{d}{dx}(e^{-x})\right)$$

$$= \frac{1}{2}(e^x + e^{-x} \cdot (-1)) = \frac{1}{2}(e^x - e^{-x}) = \sinh(x)$$

**Derivative of Hyperbolic Tangent:** $$\large \frac{d}{dx}(\tanh(x)) = 1 - \tanh^2(x)$$

**Proof Method 1 - Using Quotient Rule:** Since $\tanh(x) = \frac{\sinh(x)}{\cosh(x)}$, we apply the quotient rule:
...
**Proof Method 2 - Using Exponential Form:** Starting with $\tanh(x) = \frac{e^x - e^{-x}}{e^x + e^{-x}}$:

Let $u = e^x - e^{-x}$ and $v = e^x + e^{-x}$, so $\tanh(x) = \frac{u}{v}$

Then $u' = e^x + e^{-x} = v$ and $v' = e^x - e^{-x} = u$
...
Expanding the numerator: $$(e^x + e^{-x})^2 - (e^x - e^{-x})^2 = (e^{2x} + 2 + e^{-2x}) - (e^{2x} - 2 + e^{-2x}) = 4$$

Therefore: $$\frac{d}{dx}(\tanh(x)) = \frac{4}{(e^x + e^{-x})^2} = \frac{1}{\cosh^2(x)} = 1 - \tanh^2(x)$$
...
3. Its derivative has a simple form: 1 - tanh²(x)
...
This is similar to how the regular tangent function can be expressed in terms of sine and cosine: tan(x) =
sin(x)/cos(x).
...
Let me walk through the derivation of $$tanh(x) = (e^x - e^(-x))/(e^x + e^(-x))$$ from first principles:

1. We start with the definitions of sinh and cosh:
    - $$sinh(x) = (e^x - e^(-x))/2$$
    - $$cosh(x) = (e^x + e^(-x))/2$$
2. By definition, $$tanh(x) = sinh(x)/cosh(x)$$, so:
...
This makes tanh a useful activation function in neural networks because:

- It provides outputs centered around zero (unlike sigmoid which is centered at 0.5)
- Its range is bound to (-1, 1)
- Its derivative has a nice form: $$d/dx tanh(x) = 1 - tanh^2(x)$$
...
<div align="center">
<img src="images/AI_Programming_with_Python_ND_P2_C_2_03.png" width="600" height="auto">
...
$$softmax({zᵢ}) = e^{zᵢ} / (e^{z₁} + e^{z₂} + ... + e^{zₖ}) = e^{zᵢ} / \sum_{j=1}^{K} e^zⱼ$$
...
3. Divide each by the sum:
    - softmax(z₁) = 7.39/11.22 ≈ 0.66
    - softmax(z₂) = 2.72/11.22 ≈ 0.24
    - softmax(z₃) = 1.11/11.22 ≈ 0.10
...
because changing one input affects all outputs (due to the normalization). The derivative takes the form:
...
The softmax function is commonly denoted with $\sigma$ (sigma), especially when dealing with multi-class classification.
Here's the corrected and properly notated gradient:

**Softmax Function Definition:** $$\large \sigma_i(\mathbf{z}) = \frac{e^{z_i}}{\sum_{j=1}^{n} e^{z_j}}$$

**Gradient of Softmax (Jacobian Matrix):**

$$
\frac{\partial \sigma_i(\mathbf{z})}{\partial z_j} = \begin{cases} \sigma_i(\mathbf{z})(1 - \sigma_i(\mathbf{z})) & \text{if } i = j \\ -\sigma_i(\mathbf{z})\sigma_j(\mathbf{z}) & \text{if } i \neq j \end{cases}
$$
...
Let's denote softmax(zᵢ) as Sᵢ for simplicity.

To find $$∂Sᵢ/∂zⱼ$$, we need to consider two cases:
...
$$\begin{align}∂Sᵢ/∂zᵢ &= ∂/∂zᵢ{[e^{zᵢ} / \sum_{j=1}^{K} e^zⱼ]} \\ &= [(∂/∂zᵢ)(e^zᵢ) · ∑e^zⱼ - e^zᵢ · (∂/∂zᵢ)(∑e^zⱼ)] / (∑e^zⱼ)²\end{align}$$

The derivative of $$e^{zᵢ}$$ with respect to $$zᵢ$$ is $$e^{zᵢ}$$, and $$(∂/∂zᵢ)(∑e^zⱼ)$$ is also $$e^zᵢ$$ because only
the i-th term in the sum depends on $$zᵢ$$. So we get:

$$\begin{align} ∂Sᵢ/∂zᵢ &= [e^{zᵢ} · ∑e^{zⱼ} - e^{zᵢ} · e^{zᵢ}] / (∑e^{zⱼ})² \\ &= [e^{zᵢ} · (∑e^{zⱼ} - e^{zᵢ})] / (∑e^{zⱼ})² \\ &= [e^{zᵢ} / ∑e^{zⱼ}] · [1 - e^{zᵢ} / ∑e{^zⱼ}] \\ &= Sᵢ · (1 - Sᵢ)\end{align}$$

This gives us the first part of the formula: $$softmax(zᵢ)(1 - softmax(zᵢ))$$ when $$i = j$$.

**Case 2**: When $$i ≠ j$$ (derivative with respect to a different component)

In this case, the numerator $$e^{zᵢ}$$ doesn't directly depend on zⱼ, but the denominator $$∑e^{zⱼ}$$ does:

$$\begin{align}∂Sᵢ/∂zᵢ &= ∂/∂zᵢ{[e^{zᵢ} / \sum_{j=1}^{K} e^zⱼ]} \\ &=  [(∂/∂zⱼ)(e^{zᵢ}) · ∑e^{zⱼ} - e^{zᵢ} · (∂/∂zⱼ)(∑e^{zⱼ})] / (∑e^{zⱼ})²\end{align}$$

Since $$e^{zᵢ}$$ doesn't depend on zⱼ when i ≠ j, $$(∂/∂zⱼ)(e^{zᵢ}) = 0$$. The derivative of the sum with respect to zⱼ
is $$e^{zⱼ}$$.

This gives us:

$$
\begin{align}∂Sᵢ/∂zⱼ &= [0 · ∑e^{zⱼ} - e^{zᵢ} · e^{zⱼ}] / (∑e^{zⱼ})² \\ &= -e^{zᵢ} · e^{zⱼ} / (∑e^{zⱼ})² \\ &= -(e^{zᵢ}/ ∑e^{zⱼ}) · (e^{zⱼ} / ∑e^{zⱼ}) \\ &= -Sᵢ · Sⱼ\end{align}
$$

This gives us the second part of the formula: $$-softmax(zᵢ)softmax(zⱼ)$$ when $$i ≠ j$$.
...
The cross-entropy loss for a single example is defined as: $$L = -∑ᵢ yᵢ log(Sᵢ)$$

Where yᵢ is typically 1 for the correct class and 0 for all other classes (one-hot encoding).
...
To find $$∂L/∂zⱼ$$, we use the chain rule: $$∂L/∂zⱼ = ∑ᵢ (∂L/∂Sᵢ) · (∂Sᵢ/∂zⱼ)$$

$$\text{We know that } \frac{\partial L}{\partial S_i} = -\frac{y_i}{S_i}.$$
...
$$\text{1. The } i=j \text{ term: } \left(-\frac{y_j}{S_j}\right) \cdot S_j(1-S_j) = -y_j(1-S_j)$$

$$\text{2. All other terms where } i \neq j: \sum_{i \neq j}\left(-\frac{y_i}{S_i}\right) \cdot (-S_i \cdot S_j) = S_j \cdot \sum_{i \neq j}y_i$$

$$\text{Combining these:}$$

$$\frac{\partial L}{\partial z_j} = -y_j(1-S_j) + S_j \cdot \sum_{i \neq j}y_i$$

$$ = -y*j + y_jS_j + S_j \cdot \sum*{i \neq j}y_i$$
...
For one-hot encoded targets where $$∑ᵢ yᵢ = 1,$$ this simplifies to: $$∂L/∂zⱼ = -yⱼ + Sⱼ = Sⱼ - yⱼ = softmax(zⱼ) - yⱼ$$
...
##### Deriving Why $$∂L/∂Sᵢ = -yᵢ/Sᵢ$$

###### Starting with the Cross-Entropy Loss Function

To understand why $$∂L/∂Sᵢ = -yᵢ/Sᵢ$$, let's begin with the definition of cross-entropy loss for classification
problems:
...
- yᵢ is the true probability (typically 0 or 1 for one-hot encoded labels)
- Sᵢ is the predicted probability from the softmax function
- The sum is taken over all classes

###### Taking the Partial Derivative

To find $$∂L/∂Sᵢ$$, we need to differentiate this loss function with respect to Sᵢ (the softmax output for class i).

For any class k that isn't i, the term $$yₖ log(Sₖ)$$ doesn't depend on $$Sᵢ$$, so its derivative with respect to $$Sᵢ$$
is zero.

Therefore, we only need to focus on the term $$yᵢ log(Sᵢ)$$ in the sum:

$$∂L/∂Sᵢ = ∂/∂Sᵢ(-yᵢ log(Sᵢ))$$

Using the chain rule, we know that the derivative of -log(x) with respect to $$x$$ is $$-1/x$$. So:

$$∂L/∂Sᵢ = -yᵢ · ∂/∂Sᵢ(log(Sᵢ)) = -yᵢ · (1/Sᵢ) = -yᵢ/Sᵢ$$

This is our result: $$∂L/∂Sᵢ = -yᵢ/Sᵢ$$

###### Intuitive Explanation

The derivative tells us how a small change in $$Sᵢ$$ affects the loss function. Let's understand what $$-yᵢ/Sᵢ$$ means:

1. When yᵢ = 1 (the correct class):
    - The derivative is $$-1/Sᵢ$$
    - As Sᵢ gets smaller (prediction becomes less confident in the correct class), the derivative becomes more negative
    - This means the loss increases rapidly when the model assigns low probability to the correct class
2. When yᵢ = 0 (an incorrect class):
    - The derivative is 0
...
$$∂L/∂S₁ = -y₁/S₁ = -0/0.1 = 0 ∂L/∂S₂ = -y₂/S₂ = -1/0.7 ≈ -1.43 ∂L/∂S₃ = -y₃/S₃ = -0/0.2 = 0$$
...
with respect to the logits (z values), which is how we arrive at the beautifully simple form softmax(zⱼ) - yⱼ that makes
...
$\begin{align}&z = [-2.85, 0.86, 3.44, -0.34, -1.86, 0.10, -2.54, 0.21, 2.67, -1.13]\end{align}$
...
To derive the gradient of the categorical cross-entropy loss with respect to the logits, I'll walk through the process
step by step.

First, let's understand what we're starting with. The categorical cross-entropy loss for a dataset with N samples is
defined as:
...
Simplifying: $$\frac{\partial L_{CE}}{\partial z_i} = -y_i(1-p_i) + \sum_{k \neq i} y_kp_i$$
...
In a neural network, each logit $z_i$ is typically computed as a linear function of the final hidden layer output **h**:
...
- **Precision**: The proportion of positive predictions that are correct.
    ...
    TP₁ is true positives (correctly predicted class i)

    FP₁ is false positives (incorrectly predicted as class i).
...
- **Recall (Sensitivity)**: The proportion of actual positives that are correctly identified.
    ...
    FN₁ is false negatives (class i instances predicted as something else).
...
1. First, calculate the odds ratio:
    $$
    \frac{P(y=1|\mathbf{x})}{P(y=0|\mathbf{x})} = \frac{P(y=1|\mathbf{x})}{1-P(y=1|\mathbf{x})} = \frac{\frac{1}{1 + e^{-(\mathbf{w}^T\mathbf{x} + b)}}}{1-\frac{1}{1 + e^{-(\mathbf{w}^T\mathbf{x} + b)}}}
    $$
...
3. Now our odds ratio becomes:
   $$\frac{P(y=1|\mathbf{x})}{P(y=0|\mathbf{x})} = \frac{\frac{1}{1 + e^{-(\mathbf{w}^T\mathbf{x} + b)}}}{\frac{e^{-(\mathbf{w}^T\mathbf{x} + b)}}{1 + e^{-(\mathbf{w}^T\mathbf{x} + b)}}} = \frac{1}{e^{-(\mathbf{w}^T\mathbf{x} + b)}} = e^{\mathbf{w}^T\mathbf{x} + b}$$
...
First, multiply both sides by the denominator: $$1 = 0.5 \times (1 + e^{-z})$$ $$1 = 0.5 + 0.5e^{-z}$$

Subtract 0.5 from both sides: $$0.5 = 0.5e^{-z}$$

Divide both sides by 0.5: $$1 = e^{-z}$$

Take the natural logarithm of both sides: $$\ln(1) = \ln(e^{-z})$$

Since $\ln(1) = 0$ and $\ln(e^{-z}) = -z$: $$0 = -z$$
...
When working with a binary classification problem, we have two separate probability expressions:
...
For y = 1: $$[\sigma(\mathbf{w}^T\mathbf{x} + b)]^1 \cdot [1 - \sigma(\mathbf{w}^T\mathbf{x} + b)]^{(1-1)}$$
$$= \sigma(\mathbf{w}^T\mathbf{x} + b) \cdot [1 - \sigma(\mathbf{w}^T\mathbf{x} + b)]^0$$
$$= \sigma(\mathbf{w}^T\mathbf{x} + b) \cdot 1$$ $$= \sigma(\mathbf{w}^T\mathbf{x} + b)$$

For y = 0: $$[\sigma(\mathbf{w}^T\mathbf{x} + b)]^0 \cdot [1 - \sigma(\mathbf{w}^T\mathbf{x} + b)]^{(1-0)}$$
$$= 1 \cdot [1 - \sigma(\mathbf{w}^T\mathbf{x} + b)]^1$$ $$= 1 - \sigma(\mathbf{w}^T\mathbf{x} + b)$$
...
(1-p). Its probability mass function is: $$P(X=x) = p^x \cdot (1-p)^{(1-x)}$$
...
$$\ell(\mathbf{w}, b) = \log L(\mathbf{w}, b) = \log \prod_{i=1}^{m} P(y^{(i)}|\mathbf{x}^{(i)}; \mathbf{w}, b) = \sum_{i=1}^{m} \log P(y^{(i)}|\mathbf{x}^{(i)}; \mathbf{w}, b)$$
...
I'll walk you through how we get from the first derivative expression to the simplified form in the last line.

Looking at the first expression, we start with:
...
is the logit (pre-activation output).

To compute this derivative, we need to apply the chain rule since the loss function depends on $z^{(i)}$ through
$\hat{y}^{(i)}$.
...
2. For the second term $(1 - y^{(i)})\log(1 - \hat{y}^{(i)})$:
    ...
    the negative sign from the chain rule)
...
$$z^{(i)} = w_1x_1^{(i)} + w_2x_2^{(i)} + \cdots + w_jx_j^{(i)} + \cdots + w_nx_n^{(i)} + b$$

Now we want to find $\frac{\partial z^{(i)}}{\partial w_j}$, which means we want to know how $z^{(i)}$ changes when we
make a small change to the specific weight $w_j$.
...
$$z^{(i)} = w_1x_1^{(i)} + w_2x_2^{(i)} + \cdots + w_nx_n^{(i)} + b$$
