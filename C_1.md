...
$$\mathbf{w}*{\text{new}} \cdot \mathbf{x}*{\text{new}} = w_0x_0 + w_1x_1 + ... + w_nx_n = b \cdot 1 + w_1x_1 + ... + w_nx_n = b + w_1x_1 + ... + w_nx_n$$
...
The general form of this update rule can be expressed elegantly as:

$$\mathbf{w}*{\text{new}} = \mathbf{w}*{\text{old}} + \alpha \cdot (y - \hat{y}) \cdot \mathbf{x}$$

And for the bias:

$$b_{\text{new}} = b_{\text{old}} + \alpha \cdot (y - \hat{y})$$

Lets see the perceptron update rule in matrix form, which elegantly captures the simultaneous updates to all weights and
the bias.
...
With these definitions, the perceptron prediction becomes simply: $$\hat{y} = \text{step}(\mathbf{W}^T \mathbf{X})$$

Now, the update rule in matrix form can be written as:
$$\mathbf{W}*{\text{new}} = \mathbf{W}*{\text{old}} + \alpha(y - \hat{y})\mathbf{X}$$
...
$$\begin{bmatrix} b_{\text{new}} \ w_{1,\text{new}} \ w_{2,\text{new}} \ \vdots \ w_{n,\text{new}} \end{bmatrix} = \begin{bmatrix} b_{\text{old}} \ w_{1,\text{old}} \ w_{2,\text{old}} \ \vdots \ w_{n,\text{old}} \end{bmatrix} + \alpha(y - \hat{y}) \begin{bmatrix} 1 \ x_1 \ x_2 \ \vdots \ x_n \end{bmatrix}$$

When expanded, this gives us:

$$b_{\text{new}} = b_{\text{old}} + \alpha(y - \hat{y}) \cdot 1$$
$$w_{1,\text{new}} = w_{1,\text{old}} + \alpha(y - \hat{y}) \cdot x_1$$
$$w_{2,\text{new}} = w_{2,\text{old}} + \alpha(y - \hat{y}) \cdot x_2$$ $$\vdots$$
$$w_{n,\text{new}} = w_{n,\text{old}} + \alpha(y - \hat{y}) \cdot x_n$$

To create vertical matrices in LaTeX for both equations, you need to use the double backslash (`\\`) as line separators
rather than single backslashes. Here's how to properly format both equations:
...
The key difference between horizontal and vertical matrices in LaTeX is:

- For horizontal matrices (row vectors), elements are separated by `&` symbols
- For vertical matrices (column vectors), elements are separated by `\\` symbols

The double backslash tells LaTeX to create a new line in the matrix, effectively stacking the elements vertically rather
than placing them side by side.

Let me clear up how to properly format these equations in LaTeX for Typora.

## For Vertical Matrices

The issue you're having is with the formatting. In LaTeX, to create vertical matrices (column vectors), you need double
backslashes (`\\`) to create line breaks between elements. Here's the properly formatted code:
...
## For the Expanded Equations

For the expanded equations, they should be formatted as:
...
## Common LaTeX Matrix Issues in Typora

The code that's showing in your screenshot has some formatting issues that could be causing problems:

1. The backslashes may be escaping incorrectly when copied (appearing as `\\` when they should be rendered as a single
   `\` in the LaTeX code)
2. Some editors require extra spacing around LaTeX commands
3. The matrices might not be breaking correctly because they're too complex or long for a single line

## Alternative Approach

If you're still having trouble, you could try an alternative format with more explicit spacing:
...
Remember that in LaTeX math environments:

- `\\` creates a new line within the matrix
- Each matrix needs its own `\begin{bmatrix}` and `\end{bmatrix}` tags
- The entire equation should be enclosed in a single pair of `$$` delimiters
